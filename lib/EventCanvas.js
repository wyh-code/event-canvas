...
export default class EventCanvas extends Event {
  ...
  devicePixelRatio() {
    ...
    if (!canvas.style.width || !canvas.style.height) {
      console.error(
        '请设置 canvas 宽高：<canvas style="width:width; height:height"></canvas>',
      );
      return;
    }
  ...
  customFont(options, hideCtx, ele) {
    ...
    hideCtx.rect(options.x, options.y, options.width, options.height);
    hideCtx.fill();
    hideCtx.restore();


    console.log(`rgba(${r}, ${g}, ${b}, ${a})`, ele.id)
  }
